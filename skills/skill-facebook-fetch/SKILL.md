...
bash 邏輯。因此本機執行 `run_daily_fetch.py` 得到的 `data/`、`README.md`、
`raw_facebook_fetch.csv` 結果，跟排程觸發 GitHub Actions 的結果是**同一套程式碼路徑**，
不會有兩份邏輯各自漂移的問題。

**本技能不複製任何程式碼**：這份 skill 就放在 `Facebook.Fetch` repo 內
（`skills/skill-facebook-fetch/`），跟它描述的 `scripts/run_daily_fetch.py`、
`tools/update_fb_cookie.py`、`fetch_facebook_posts.py`、`fetch_saved_list.py`
在同一個 repo、同一份 git 歷史裡，所以下面所有指令都直接引用 repo 根目錄的路徑，
不存在需要另外同步的副本。（若未來要把這個 skill 搬到 `wenchiehlee/skills` 登錄庫供其他 repo
重用，那份**才**需要照登錄庫規則內附自包含的腳本副本 —— 那是另一份獨立維護的技能定義，
不是本檔案。）
...
| 情境 | 指令 |
|---|---|
| 更新過期的 FB_COOKIE / FB_DTSG | 見下方「更新 Cookie 流程」；腳本 `tools/update_fb_cookie.py` |
...
python scripts/run_daily_fetch.py --cookie "$FB_COOKIE" --fb-dtsg "$FB_DTSG"
...
# 回溯抓近 30 天（補資料）
python scripts/run_daily_fetch.py --cookie "$FB_COOKIE" --fb-dtsg "$FB_DTSG" --months-back 1
```
